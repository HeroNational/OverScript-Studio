...
WizardStyle=modern
SetupIconFile=windows\assets\app_icon.ico
...
; Include assets only if you need them outside the packaged app; otherwise omit to reduce installer size.
...
; Avoid deleting user files in the app directory during install.
...
begin
  ...
  begin
    MsgBox('OverScript Studio a ete installe avec succes!', mbInformation, MB_OK);
  end;
end;
...
begin
  Result := True;
  { Vous pouvez ajouter des verifications systeme ici si necessaire }
end;
